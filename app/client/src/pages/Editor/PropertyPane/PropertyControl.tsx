...
const PropertyControl = memo((props: Props) => {
      ...
      const consolidatedUpdates = updates
        ...
        .reduce(
          (
            ...
            const findWidgitIndex = acc.findIndex(
              (val) => val.widgetId === curr.widgetId,
            );
            if (findWidgitIndex >= 0) {
              //merge updates of the same widget
              const mergeCopy = merge({}, acc[findWidgitIndex], curr);
              acc[findWidgitIndex] = mergeCopy;
            } else {
              acc.push(curr);
            }
            return acc;
  ...
  const onPropertyChange = useCallback(
    (
      ...
      const allPropertiesToUpdates = getPropertyUpdatesWithAssociatedWidgetUpdates(
        propertyName,
        propertyValue,
      );
        ...
        >
          ...
          {PropertyControlFactory.createControl(
            config,
            {
              ...
              onBatchUpdateWithAssociatedUpdates: onBatchUpdateWithAssociatedWidgetUpdates,
              theme: props.theme,
            },
            isDynamic,
            customJSControl,
